...
function RowItemRenderer(props) {
    ...
    return <Vertical color={"light"} brightness={rowIsSelected ? -0.5 : 0.5}>
        ...
        <Vertical $visible={$showDetail}>
            <AutoPopulateColumnTable $data={$detailData} dataKey={data => data?.id_}/>
        </Vertical>
    </Vertical>
}
...
export default function AutoPopulateColumnTable({dataKey, $data, domRef, $value, $error, onChange, ...props}) {
